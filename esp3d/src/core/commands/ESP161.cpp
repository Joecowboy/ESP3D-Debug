...
void ESP3DCommands::ESP161(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    intValue = ESP3DSettings::readUint32(static_cast<int>(ESP3DSettingIndex::esp3d_websocket_port));
    ...
    esp3d_log(
        ...
        intValue,
        ESP3DSettings::isValidIntegerSetting(intValue, ESP3DSettingIndex::esp3d_websocket_port));
    if (ESP3DSettings::isValidIntegerSetting(intValue, ESP3DSettingIndex::esp3d_websocket_port)) {
      esp3d_log("Value %ld is valid", intValue);
      if (!ESP3DSettings::writeUint32(static_cast<int>(ESP3DSettingIndex::esp3d_websocket_port), intValue)) {
        hasError = true;
