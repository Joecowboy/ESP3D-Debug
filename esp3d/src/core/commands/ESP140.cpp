...
void ESP3DCommands::ESP140(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  const ESP3DSettingIndex settingIndex[] = {
      ...
      ESP3DSettingIndex::esp3d_time_zone,
      ESP3DSettingIndex::esp3d_internet_time,
      (ESP3DSettingIndex)-1,
  };
  ...
  if (tmpstr.length() == 0) {
    ...
    for (uint8_t i = 0; i < cmdListSize - 2; i++) {
      ...
      ok_msg += ESP3DSettings::readString(static_cast<int>(settingIndex[i]));
      if (json) {
        ok_msg += "\"";
      } else {
      }
    ...
    ok_msg += ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_internet_time)) ? "yes" : "no";
    if (json) {
      ok_msg += "\"";
    } else {
    }
    ...
    for (uint8_t i = 0; i < cmdListSize; i++) {
      ...
      if (tmpstr.length() != 0) {
        ...
        if (settingIndex[i] == (ESP3DSettingIndex)-1) {
        ...
        } else if (settingIndex[i] == ESP3DSettingIndex::esp3d_internet_time) {
          tmpstr.toLowerCase();
          if (tmpstr == "yes" || tmpstr == "no" || tmpstr == "1" ||
            ...
            if (!ESP3DSettings::writeByte(static_cast<int>(settingIndex[i]), val)) {
              hasError = true;
          ...
          if (ESP3DSettings::isValidStringSetting(tmpstr.c_str(),
            ...
            if (!ESP3DSettings::writeString(static_cast<int>(settingIndex[i]), tmpstr.c_str())) {
              hasError = true;
              error_msg = "Set value failed";
            }
          } else {
