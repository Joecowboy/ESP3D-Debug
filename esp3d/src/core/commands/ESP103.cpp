...
void ESP3DCommands::ESP103(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  const ESP3DSettingIndex settingIndex[] = {
      ESP3DSettingIndex::esp3d_sta_ip_value,
      ESP3DSettingIndex::esp3d_sta_mask_value,
      ESP3DSettingIndex::esp3d_sta_gateway_value,
      ESP3DSettingIndex::esp3d_sta_dns_value};
  ...
  if (tmpstr.length() == 0) {
    ...
    ok_msg += ESP3DSettings::readIPString(static_cast<int>(ESP3DSettingIndex::esp3d_sta_ip_value));
    if (json) {
      ok_msg += "\",\"gw\":\"";
    } else {
      ok_msg += ", GW: ";
    }
    ok_msg += ESP3DSettings::readIPString(static_cast<int>(ESP3DSettingIndex::esp3d_sta_gateway_value));
    if (json) {
      ok_msg += "\",\"msk\":\"";
    } else {
      ok_msg += ", MSK: ";
    }
    ok_msg += ESP3DSettings::readIPString(static_cast<int>(ESP3DSettingIndex::esp3d_sta_mask_value));
    if (json) {
      ok_msg += "\",\"dns\":\"";
    } else {
      ok_msg += ", DNS: ";
    }
    ok_msg += ESP3DSettings::readIPString(static_cast<int>(ESP3DSettingIndex::esp3d_sta_dns_value));
    if (json) {
      ok_msg += "\"}";
    }
  } else {
    ...
    for (uint8_t i = 0; i < cmdListSize; i++) {
      ...
      if (tmpstr.length() != 0) {
        ...
        if (ESP3DSettings::isValidIPStringSetting(tmpstr.c_str(),
          ...
          if (!ESP3DSettings::writeIPString(static_cast<int>(settingIndex[i]), tmpstr.c_str())) {
            hasError = true;
