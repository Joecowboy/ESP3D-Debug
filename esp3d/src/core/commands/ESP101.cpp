...
void ESP3DCommands::ESP101(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    ...
    esp3d_log(
        ...
        tmpstr.c_str(),
        ESP3DSettings::isValidStringSetting(tmpstr.c_str(), ESP3DSettingIndex::esp3d_sta_password));
    if (ESP3DSettings::isValidStringSetting(tmpstr.c_str(), ESP3DSettingIndex::esp3d_sta_password)) {
      esp3d_log("Value %s is valid", tmpstr.c_str());
      if (!ESP3DSettings::writeString(static_cast<int>(ESP3DSettingIndex::esp3d_sta_password), tmpstr.c_str())) {
        hasError = true;
