...
void ESP3DCommands::ESP181(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    ...
    intValue = ESP3DSettings::readUint32(static_cast<int>(ESP3DSettingIndex::esp3d_ftp_ctrl_port));
    ...
    intValue = ESP3DSettings::readUint32(static_cast<int>(ESP3DSettingIndex::esp3d_ftp_data_active_port));
    ok_msg += String(intValue);
    if (json) {
      ok_msg += "\",\"passive\":\"";
    } else {
    ...
    intValue = ESP3DSettings::readUint32(static_cast<int>(ESP3DSettingIndex::esp3d_ftp_data_passive_port));
    ok_msg += String(intValue);
    if (json) {
      ok_msg += "\"}";
    }
    ...
    if (tmpstr.length() > 0) {
      ...
      if (ESP3DSettings::isValidIntegerSetting(intValue, ESP3DSettingIndex::esp3d_ftp_ctrl_port)) {
        esp3d_log("Value %ld is valid", intValue);
        if (!ESP3DSettings::writeUint32(static_cast<int>(ESP3DSettingIndex::esp3d_ftp_ctrl_port), intValue)) {
          hasError = true;
    ...
    if (!hasError) {
      ...
      if (tmpstr.length() > 0) {
        ...
        if (ESP3DSettings::isValidIntegerSetting(intValue,
                                                 ESP3DSettingIndex::esp3d_ftp_data_active_port)) {
          esp3d_log("Value %ld is valid", intValue);
          if (!ESP3DSettings::writeUint32(static_cast<int>(ESP3DSettingIndex::esp3d_ftp_data_active_port), intValue)) {
            hasError = true;
            error_msg = "Set value failed";
          }
        } else {
    ...
    if (!hasError) {
      ...
      if (tmpstr.length() > 0) {
        ...
        intValue = atoi(tmpstr.c_str());
        if (ESP3DSettings::isValidIntegerSetting(intValue,
                                                 ESP3DSettingIndex::esp3d_ftp_data_passive_port)) {
          esp3d_log("Value %ld is valid", intValue);
          if (!ESP3DSettings::writeUint32(static_cast<int>(ESP3DSettingIndex::esp3d_ftp_data_passive_port),
                                          intValue)) {
            hasError = true;
            error_msg = "Set value failed";
          }
