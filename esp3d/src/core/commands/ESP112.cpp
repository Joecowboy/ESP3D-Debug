...
void ESP3DCommands::ESP112(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    const ESP3DSettingDescription* settingPtr =
        ESP3DSettings::getSettingPtr(ESP3DSettingIndex::esp3d_hostname);
    if (settingPtr) {
      ok_msg = ESP3DSettings::readString(static_cast<int>(ESP3DSettingIndex::esp3d_hostname));
    } else {
      hasError = true;
    ...
    esp3d_log(
        ...
        tmpstr.c_str(),
        ESP3DSettings::isValidStringSetting(tmpstr.c_str(), ESP3DSettingIndex::esp3d_hostname));
    if (ESP3DSettings::isValidStringSetting(tmpstr.c_str(), ESP3DSettingIndex::esp3d_hostname)) {
      esp3d_log("Value %s is valid", tmpstr.c_str());
      if (!ESP3DSettings::writeString(static_cast<int>(ESP3DSettingIndex::esp3d_hostname), tmpstr.c_str())) {
        hasError = true;
