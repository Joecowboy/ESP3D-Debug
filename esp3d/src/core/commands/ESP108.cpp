...
void ESP3DCommands::ESP108(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    byteValue = ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_ap_channel));
    ...
    esp3d_log("got %s param for a value of %d, is valid %d", tmpstr.c_str(),
              byteValue,
              ESP3DSettings::isValidByteSetting(byteValue, ESP3DSettingIndex::esp3d_ap_channel));
    if (ESP3DSettings::isValidByteSetting(byteValue, ESP3DSettingIndex::esp3d_ap_channel)) {
      esp3d_log("Value %d is valid", byteValue);
      if (!ESP3DSettings::writeByte(static_cast<int>(ESP3DSettingIndex::esp3d_ap_channel), byteValue)) {
        hasError = true;
