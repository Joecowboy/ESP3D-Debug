...
void ESP3DCommands::ESP190(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  ESP3DState setting_mode = (ESP3DState)ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_webdav_on));
  if (tmpstr.length() == 0) {
...
#endif  // AUTHENTICATION_FEATURE
    if (stateON || stateOFF) {
      if (!ESP3DSettings::writeByte(static_cast<int>(ESP3DSettingIndex::esp3d_webdav_on), stateOFF ? 0 : 1)) {
        hasError = true;
