...
void ESP3DCommands::ESP150(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    ...
    ok_msg += String(ESP3DSettings::readUint32(static_cast<int>(ESP3DSettingIndex::esp3d_boot_delay)));
    if (json) {
      ok_msg += "\",\"verbose\":\"";
    } else {
      ok_msg += ", verbose=";
    ...
    if (tmpstr.length() != 0) {
      ...
      if (ESP3DSettings::isValidIntegerSetting(tmpstr.toInt(),
                                               ESP3DSettingIndex::esp3d_boot_delay)) {
        if (!ESP3DSettings::writeUint32(static_cast<int>(ESP3DSettingIndex::esp3d_boot_delay), tmpstr.toInt())) {
          hasError = true;
    ...
    if (!hasError) {
      ...
      if (tmpstr.length() != 0) {
        ...
        if (tmpstr == "ON" || tmpstr == "OFF" || tmpstr == "1" ||
          ...
          if (!ESP3DSettings::writeByte(static_cast<int>(ESP3DSettingIndex::esp3d_verbose_boot), val)) {
            hasError = true;
            error_msg = "Set verbose failed";
            esp3d_log_e("%s", error_msg.c_str());
          }
        } else {
          hasError = true;
