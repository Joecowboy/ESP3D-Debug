...
void ESP3DCommands::ESP800(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (setupparam.length() > 0) {
    if (!ESP3DSettings::writeByte(static_cast<int>(ESP3DSettingIndex::esp3d_setup), setupparam == "1" ? 1 : 0)) {
      // not blocking error
  ...
  // Setup done
  tmpstr = ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_setup)) == 0 ? "Enabled" : "Disabled";
