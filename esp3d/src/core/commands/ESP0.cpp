...
*/
#include <Arduino.h> // Added for String class
...
void ESP3DCommands::ESP0(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    ...
    for (uint i = 0; i < cmdNb; i++) {
      ...
      if (!dispatch(nullptr, tmpstr.c_str(), target, requestId, ESP3DMessageType::core, ESP3DAuthenticationLevel::guest)) {
    ...
    if (json) {
      if (!dispatch(nullptr, "]}", target, requestId, ESP3DMessageType::tail, ESP3DAuthenticationLevel::guest)) {
        esp3d_log_e("Error sending answer to clients");
        return;
      }
    } else {
      if (!dispatch(nullptr, "ok\n", target, requestId, ESP3DMessageType::tail, ESP3DAuthenticationLevel::guest)) {
        esp3d_log_e("Error sending answer to clients");
        return;
      }
    }
