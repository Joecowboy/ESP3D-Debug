...
void ESP3DCommands::ESP120(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  ESP3DState setting_http_mode =
      (ESP3DState)ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_http_on));
  if (tmpstr.length() == 0) {
    ...
    if (tmpstr == "OFF" || tmpstr == "ON") {
      if (!ESP3DSettings::writeByte(static_cast<int>(ESP3DSettingIndex::esp3d_http_on), tmpstr == "OFF" ? 0 : 1)) {
        hasError = true;
