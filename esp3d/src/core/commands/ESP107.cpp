...
void ESP3DCommands::ESP107(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    ok_msg = ESP3DSettings::readIPString(static_cast<int>(ESP3DSettingIndex::esp3d_ap_ip_value));
  } else {
#if defined(AUTHENTICATION_FEATURE)
    ...
    esp3d_log(
        ...
        tmpstr.c_str(),
        ESP3DSettings::isValidIPStringSetting(tmpstr.c_str(), ESP3DSettingIndex::esp3d_ap_ip_value));
    if (ESP3DSettings::isValidIPStringSetting(tmpstr.c_str(),
                                              ESP3DSettingIndex::esp3d_ap_ip_value)) {
      esp3d_log("Value %s is valid", tmpstr.c_str());
      if (!ESP3DSettings::writeIPString(static_cast<int>(ESP3DSettingIndex::esp3d_ap_ip_value), tmpstr.c_str())) {
        hasError = true;
