/*
 ESP510.cpp - ESP3D command class
...
void ESP3DCommands::ESP510(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    const ESP3DSettingDescription* settingPtr =
        ESP3DSettings::getSettingPtr(ESP3DSettingIndex::esp3d_session_timeout);
    if (settingPtr) {
      ok_msg = String(ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_session_timeout)));
    } else {
      hasError = true;
    ...
    if (ESP3DSettings::isValidByteSetting((uint8_t)tmpstr.toInt(),
      ...
      if (!ESP3DSettings::writeByte(static_cast<int>(ESP3DSettingIndex::esp3d_session_timeout),
