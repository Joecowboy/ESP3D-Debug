...
void ESP3DCommands::ESP160(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  ESP3DState setting_mode =
      (ESP3DState)ESP3DSettings::readByte((int)ESP3DSettingIndex::esp3d_websocket_on);
  if (tmpstr.length() == 0) {
...
#endif  // AUTHENTICATION_FEATURE
    if (stateON || stateOFF) {
      if (!ESP3DSettings::writeByte((int)ESP3DSettingIndex::esp3d_websocket_on, stateOFF ? 0 : 1)) {
        hasError = true;
