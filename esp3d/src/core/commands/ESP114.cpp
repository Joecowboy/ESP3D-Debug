...
void ESP3DCommands::ESP114(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    byteValue = ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_boot_radio_state));
    ...
    esp3d_log(
        ...
        byteValue,
        ESP3DSettings::isValidByteSetting(byteValue, ESP3DSettingIndex::esp3d_boot_radio_state));
    if (ESP3DSettings::isValidByteSetting(byteValue, ESP3DSettingIndex::esp3d_boot_radio_state)) {
      esp3d_log("Value %d is valid", byteValue);
      if (!ESP3DSettings::writeByte(static_cast<int>(ESP3DSettingIndex::esp3d_boot_radio_state), byteValue)) {
        hasError = true;
