...
void ESP3DCommands::ESP550(int cmd_params_pos, ESP3DMessage* msg) {
  ...
  if (tmpstr.length() == 0) {
    const ESP3DSettingDescription* settingPtr =
        ESP3DSettings::getSettingPtr(ESP3DSettingIndex::esp3d_admin_pwd);
    if (settingPtr) {
      ok_msg = ESP3DSettings::readString(static_cast<int>(ESP3DSettingIndex::esp3d_admin_pwd));
    } else {
      hasError = true;
    ...
    esp3d_log(
        ...
        tmpstr.c_str(),
        ESP3DSettings::isValidStringSetting(tmpstr.c_str(), ESP3DSettingIndex::esp3d_admin_pwd));
    if (ESP3DSettings::isValidStringSetting(tmpstr.c_str(), ESP3DSettingIndex::esp3d_admin_pwd)) {
      esp3d_log("Value %s is valid", tmpstr.c_str());
      if (!ESP3DSettings::writeString(static_cast<int>(ESP3DSettingIndex::esp3d_admin_pwd), tmpstr.c_str())) {
        hasError = true;
