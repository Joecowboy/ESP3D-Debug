...
bool mDNS_Service::begin(const char* hostname) {
  ...
  if (WiFi.getMode() != WIFI_AP) {
    ...
    esp3d_log("Start mDNS for %s", _hostname.c_str());
    if (!MDNS.begin(_hostname.c_str())) {
      String errorMsg = "mDNS failed to start";
      esp3d_commands.dispatch((uint8_t*)errorMsg.c_str(), errorMsg.length(), ESP3DClientType::all_clients,
                              {0}, ESP3DMessageType::unique, ESP3DClientType::system,
...
#if defined(ARDUINO_ARCH_ESP8266)
// callback function for mDNS service query
