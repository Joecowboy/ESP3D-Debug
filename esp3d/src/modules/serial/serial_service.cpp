...
void ESP3DSerialService::setParameters() {
  ...
  _needauthentication =
      (ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_secure_serial)) == 0) ? false : true;
#else
...
bool ESP3DSerialService::dispatch(ESP3DMessage *message) {
  bool done = false;
  // Only if serial service is started
  if (_started) {
...
#endif  // COMMUNICATION_PROTOCOL == RAW_SERIAL || defined(ESP_SERIAL_BRIDGE_OUTPUT) || COMMUNICATION_PROTOCOL == MKS_SERIAL