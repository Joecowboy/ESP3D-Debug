...
bool Display::displaySignal(bool force) {
  ...
  static String label;
	...
	if (mode == WIFI_AP || (mode == WIFI_APSTA && !WiFi.isConnected())) {
	  // AP-only or APSTA with no STA connection
	  if (sig != 100) {
		sig = 100;
		refresh_signal = true;
	  }
	  String apLabel = WiFiConfig::AP_SSID();
	  if (label != apLabel) {
		label = apLabel;
		refresh_label = true;
	  }
	} else if (WiFi.isConnected()) {
	  // STA-only or APSTA with STA connection
	  int sigtmp = WiFiConfig::getSignal(WiFi.RSSI(), false);
	  if (sig != sigtmp) {
		sig = sigtmp;
		refresh_signal = true;
	  }
	  String staLabel = WiFi.SSID();
	  if (label != staLabel) {
		label = staLabel;
		refresh_label = true;
	  }
...
bool Display::displayIP(bool force) {
  ...
  case ESP_WIFI_APSTA:
    // Option 1: Return STA IP (most common use)
    s = WiFi.localIP().toString();

    // Option 2: Return both IPs (if needed)
    // s = WiFi.localIP().toString() + " / " + WiFi.softAPIP().toString();
    break;
#endif  // WIFI_FEATURE
#if defined(ETH_FEATURE)
      case ESP_ETH_STA:
