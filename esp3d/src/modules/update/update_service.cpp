...
bool processingFileFunction(const char* section, const char* key,
  ...
  if (strcasecmp("network", section) == 0) {
	...
	// Radio mode BT, WIFI-STA, WIFI-AP, WIFI-AP+STA, WIFI-SETUP, ETH-STA, OFF
	if (!done) {
	  if (strcasecmp("radio_mode", key) == 0) {
		...
		if (strcasecmp("BT", value) == 0) {
		  ...
		  b = ESP_WIFI_AP;
		} else if (strcasecmp("WIFI-AP+STA", value) == 0) {
		  b = ESP_WIFI_APSTA;
    ...
    // Target Firmware None / Marlin / Klipper / Repetier / MarlinKimbra / Smoothieware /
    // GRBL
    if (!done) {
      if (strcasecmp("TargetFW", key) == 0) {
        ...
        if (strcasecmp("None", value) == 0) {
          ...
          b = MARLIN;
		} else if (strcasecmp("KLIPPER", value) == 0) {
          b = KLIPPER;
