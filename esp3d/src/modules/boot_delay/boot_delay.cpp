...
bool BootDelay::begin() {
  _totalduration = ESP3DSettings::readUint32(static_cast<int>(ESP3DSettingIndex::esp3d_boot_delay));
  esp3d_log("Boot delay %d", _totalduration);
  if (!ESP3DSettings::isValidIntegerSetting(_totalduration, ESP3DSettingIndex::esp3d_boot_delay)) {
    _totalduration = ESP3DSettings::getDefaultIntegerSetting(ESP3DSettingIndex::esp3d_boot_delay);
