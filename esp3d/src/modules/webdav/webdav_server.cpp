...
bool WebdavServer::begin() {
  ...
  if (ESP3DSettings::readByte(static_cast<int>(ESP3DSettingIndex::esp3d_webdav_on)) != 1) {
    return true;
  }
  // Get webdav port
  _port = ESP3DSettings::readUint32(static_cast<int>(ESP3DSettingIndex::esp3d_webdav_port));
...
void WebdavServer::parseRequest() {
  ...
  _headers.clear();
  // read the first line of the request to get the request method, URL and HTTP
